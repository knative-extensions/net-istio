...
func TestReconcile_EnableAutoTLS(t *testing.T) {
	table := TableTest{{
		...
		Objects: []runtime.Object{
			...
			addLabelsToSecret(originSecret("knative-serving", "secret0"), map[string]string{networking.CertificateUIDLabelKey: "my-certificate"}),
			ingressService,
		},
		WantCreates: []runtime.Object{
			// The newly created per-Ingress Gateway.
			...
			targetSecret("istio-system", targetSecretName, map[string]string{
				networking.CertificateUIDLabelKey:        "my-certificate",
...
func newTestSetup(t *testing.T, configs ...*corev1.ConfigMap) (
	...
	*configmap.ManualWatcher) {
	...
	ctx, cancel, informers := SetupFakeContextWithCancel(t, informerfiltering.GetContextWithFilteringLabelSelector)
...
func TestGlobalResyncOnUpdateGatewayConfigMap(t *testing.T) {
	cases := []struct {
		name                    string
		enableInformerFiltering bool
	}{{
		"Disable Informer Filtering",
		false,
	},
		{
			"Enable Informer Filtering",
			true,
		}}

	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			if c.enableInformerFiltering {
				t.Setenv(informerfiltering.EnableSecretInformerFilteringByCertUIDEnv, "true")
			}
