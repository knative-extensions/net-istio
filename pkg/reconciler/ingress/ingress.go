...
func (r *Reconciler) reconcileIngress(ctx context.Context, ing *v1alpha1.Ingress) error {
	...
	if r.shouldReconcileTLS(ctx, ing) {
		...
		ingressGateways, err := resources.MakeIngressGateways(ctx, ing, nonWildcardIngressTLS, nonWildcardSecrets, r.svcLister)
		if err != nil {
			return err
		}
		...
		// When auto TLS is enabled, the VirtualService will not be attached to the global Gateways.
		// Instead, the VirtualService will be attached to the Gateway generated by Knative.
		gatewayNames[v1alpha1.IngressVisibilityExternalIP] = sets.NewString(resources.GetQualifiedGatewayNames(ingressGateways)...)
